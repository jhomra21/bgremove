...
Prefer one-way dependencies. UI code may depend on `src/browser/`. Browser and native engines may depend on `src/core/`. CLI and the public Node API may depend on the native engine. Reusable native behavior must not depend on the CLI layer.
